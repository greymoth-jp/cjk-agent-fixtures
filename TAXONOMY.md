...
The seven failure modes in this repository are not seven unrelated bugs. They
...
Three friction types account for all seven fixtures.

| Type | Name | Fixtures here |
|---|---|---|
| 1 | Format mismatch | 3 |
| 2 | Legal / schema mismatch | 0 |
| 3 | Cultural / UX mismatch | 4 |
...
> If a system assumes one character is one byte and one display column in a
> single canonical encoding, then text is cut mid-character, cursors land in the
> wrong column, and two equal strings compare unequal, whenever the input is CJK,
> Hangul, RTL, or carries combining marks.
...
> harmless, and text flows left to right, then the key that confirms an IME
> candidate is read as submit, pending composition is dropped on focus change,
> the field freezes after refocus, or a line lays out backwards, in context Z
> being a user typing through an IME (kana, Hangul, pinyin) or entering a
...
| Type | Fixtures |
|---|---|
| 1: Format mismatch | 3 (#2, #3, #7) |
| 2: Legal / schema mismatch | 0 |
| 3: Cultural / UX mismatch | 4 (#1, #4, #5, #6) |
| **Total** | **7** |

Scripts touched across the set: Japanese (kana / kanji), Korean (Hangul),
Chinese (pinyin), Arabic and Hebrew (RTL), and Latin and Arabic combining marks.
Both language ports (Vitest and `go test`) carry the same seven cases.
